...
MAX_LEN = 128      # Optimized based on Text Length Distribution EDA
BATCH_SIZE = 16
EPOCHS = 5         
LEARNING_RATE = 2e-5
...
def train_subtask_1():
    ...
    # Load dataset from your directory structure
    df = pd.read_csv('../data/c2a/c2a_trial.csv', sep=';')
    ...
    # Weighted Loss: 3.0 weight for 'TRUE' (C2A) based on label distribution EDA
...
if __name__ == "__main__":
    if os.path.exists('../data/c2a/c2a_trial.csv'):
        train_subtask_1()
    else:
        print("Error: c2a_trial.csv not found in the expected directory.")