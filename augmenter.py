...
def augment_positive_samples(input_file, output_file):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        if row[label_col] == True:  # Only augment positive samples
            augmented_text = back_translation_aug.augment(row[text_col])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_file', type=str, required=True, help='Path to save the augmented CSV file')
